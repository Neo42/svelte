...
<script>
	...
	const tutorial_repo_link =
		"https://github.com/Neo42/svelte/tree/master/site/content/tutorial";
	...
	function handle_change(event) {
		completed = event.detail.components.every((file, i) => {
			...
			return (
				...
				file.source.trim().replace(/\s+$/gm, "") ===
					expected.source.trim().replace(/\s+$/gm, "")
			);
		});
	}
</script>
...
<svelte:head>
	...
	<meta
		...
		content="{selected.section.title} / {selected.chapter.title}" />
	<meta
		name="Description"
		content="{selected.section.title} / {selected.chapter.title}" />
</svelte:head>
		...
		<div class="tutorial-text">
			...
			<div class="chapter-markup" bind:this={scrollable}>
				...
				<div class="controls">
					...
					{#if selected.next}
						<a class="next" href="tutorial/{selected.next.slug}">下一节</a>
					{/if}
				</div>

				<div class="improve-chapter">
					<a class="no-underline" href={improve_link}>编辑这一节</a>
				</div>
			</div>
		</div>
